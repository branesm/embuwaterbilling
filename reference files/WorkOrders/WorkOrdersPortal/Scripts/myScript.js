...
app.config(function ($routeProvider) {
    $routeProvider
        ...
        .when('/ViewReports', {
            templateUrl: '/Home/ViewReports',
            controller: 'ctrl_viewReports'
        }) 
        ...
        .when('/CompleteWorkOrders', {
            templateUrl: '/Home/CompleteWorkOrders',
            controller: 'ctrl_readingCodes'
        })
});
...
app.controller('ctrl_viewReports', function ($scope, $log) {
    var self = this;
    self.Employees = {};

    self.GetEmployees = function (period) {
        $.ajax({
            type: "GET",
            url: "/Process/GetEmployees",
    ...
    self.GetRPTCategories = function (period) {
        $.ajax({
            type: "GET",
            url: "/Process/GetRPTCategories",
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                self.rptcodes = JSON.parse(result.json);
                $scope.$apply();
            }
        });
    }
    ...
    self.Reports = [];
    var employee = "";
    self.GetReports = function (employee, rptcode) {
        console.log(employee);
        self.Readings = [];
        $.ajax({
            type: "GET",
            url: "/Process/GetReports",
            contentType: "application/json; charset=utf-8",
            data: { 'employee': employee, 'rptcode': rptcode, 'fromdate': $("#fromdate").val(), 'todate': $("#todate").val() },
            success: function (result) {
                console.log(result);
                ...
                table.on('select', function (e, dt, type, indexes) {
                    if (type === 'row') {
                        ...
                        if (myData == null) {
                            ...
                            $('#mapLink').html('<a href ="https://www.google.com/maps/place/' + myData.latitude + ',%20' + myData.longitude + '" target="_blank">View Map </a>');
                            getImage(myData.ImageName);
                ...
                $('#photo').click(function () {
                    $.ajax({
                        type: "GET",
                        url: "/Process/getSavedImage/" + imagename,
                        success: function (result) {
                            let data = 'data:image/png;base64,' + result;
...
app.controller('ctrl_allocateDevice', function ($scope, $log) {
    ...
    self.UpdateDeviceAllocation = function () {
        $.ajax({
            ...
            success: function (result) {
                if ((result.json).includes("Success") && (result.updatetecstatus).includes("Success")) {
...
app.controller('ctrl_manageEmployees', function ($scope, $log) {
    ...
    self.DbIdentifiers = {};
    self.GetDbIdentifiers = function () {
        $.ajax({
            type: "GET",
            url: "/Process/GetDbIdentifiers",
            success: function (result) {
                self.DbIdentifiers = JSON.parse(result.json);
                $scope.$apply();
            }
        });
    }
    ...
    self.getmajicsTechnicians = function (dbName) {
        $.ajax({
            type: "GET",
            url: "/Process/getmajicsTechnicians",
            contentType: "application/json; charset=utf-8",
    ...
    self.RegisterEmployee = function () {
        ...
        var majicsEmployee = self.employee.majicsemployee;
        self.employee.emp_id = majicsEmployee.EmpID;
        self.employee.emp_name = majicsEmployee.EmpName;
        self.employee.emp_phone = majicsEmployee.EmpTel;
        console.log(self.employee);
        $.ajax({
            type: "POST",
    ...
    self.populateEmployeeData = function (data) {
        ...
        $("#registeremployee").hide("fast");
        $("#mjicstechnicians").hide("fast");
        $("#databases").hide("fast");
        $("#updateemployee").show("fast");
        $("#employeeheader").text("Update Employee " + data.Emp_ID + " - " + data.Emp_Name);
    }

    self.employeeup = {};
    self.updateEmployee = function () {
        self.employeeup.devicetag = $("#employeeedevice").val();
        self.employeeup.emp_id = self.employee.Emp_ID;
        self.employeeup.emp_password = self.employee.emp_password;
        self.employeeup.emp_email = self.employee.emp_email;
        self.employeeup.dbcode = self.employee.DbCode;
        console.log(self.employeeup);
        $.ajax({
            type: "POST",
            url: "/Process/updateEmployee",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(self.employeeup),
            success: function (result) {
                console.log(result);
                alert(result.json);
                location.reload();
    ...
    self.ShowRegisterEmployee = function () {
        $("#registerDiv").show("fast");
        $("#databases").show("fast");
        $("#registeremployee").show("fast");
        $("#mjicstechnicians").show("fast");
        $("#updateemployee").hide("fast");
        $("#employeeheader").text("Register Employee");
...
app.controller('ctrl_manageTasks', function ($scope, $log) {
    ...
    self.getcompleteworkOrders = function () {
        $("#showphotodiv").hide("fast");
        console.log("getting the work orders");
        $.ajax({
            type: "GET",
            url: "/Process/getcompleteworkOrders",
            success: function (result) {
                console.log(result.query);
                self.Devices = JSON.parse(result.json);
                dataset = self.Devices;
                var table = $('#taskstable').DataTable({
                    ...
                    columns: [
                        ...
                        
                        {
                             data: null,
                             render: function (data, type, row) {
                                 return '<a  id="navigate">Show Map</a>';
                             }
                        },
                         {
                             data: null,
                             render: function (data, type, row) {
                                 return '<a  id="showphoto">Show Photo</a>';
                             }
                         }
                    ],
                    "bDestroy": true
                ...
                $('#taskstable tbody').on('click', '#navigate', function () {
                    var data = table.row($(this).parents('tr')).data();
                    window.open('https://www.google.com/maps/place/' + data.northings + ',%20' + data.eastings+' ', '_blank');
                });

                var imagename = " ";
                $('#taskstable tbody').on('click', '#showphoto', function () {
                    var data = table.row($(this).parents('tr')).data();
                    $("#showphotodiv").show("fast");
                    console.log(data.Workorderid + "_" + data.Status);
                    imagename = data.Workorderid + "_" + data.Status;
                    //work here..  open the photo in a new tab
                    getImage(data.Workorderid + "_" + data.Status);
                });
    ...
    $("#showphotodiv").hide("fast");
    self.filterorders = {};
    self.filtercompleteWorkOrders = function () {
        $("#showphotodiv").hide("fast");
        self.filterorders.fromdate = $("#fromdate").val();
        self.filterorders.todate = $("#todate").val();
        console.log(self.filterorders);
        $.ajax({
            type: "POST",
            url: "/Process/filtercompleteWorkOrders",
            contentType: "application/json; charset=utf-8",
